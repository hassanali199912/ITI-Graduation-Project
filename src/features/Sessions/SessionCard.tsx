  ...
  return (
    <Paper elevation={3} className="p-4 rounded-xl space-y-4">
      ...
      <div dir="rtl" className="text-right space-y-1">
        ...
        <Typography><strong>الوصف:</strong> {session.description || "لا يوجد وصف"}</Typography>
        <Typography><strong>المادة:</strong> {session.subject}</Typography>
        <Typography><strong>عدد النقاط:</strong> {session.pointsAmount}</Typography>
        <Typography><strong>المدة المتوقعة:</strong> {session.estimatedDurationMinutes} دقيقة</Typography>
        <Typography><strong>تاريخ القبول:</strong> {session.acceptedAt ? new Date(session.acceptedAt).toLocaleString("ar-EG") : "لم يتم القبول بعد"}</Typography>
      </div>
