...
export default function StepTwo({ data, updateData, triggerSubmit }: Props) {
  ...
  const formData = watch();
  React.useEffect(() => {
    // console.log('StepTwo Errors:', JSON.stringify(errors, null, 2));
    console.log('StepTwo Form Data:', JSON.stringify(formData, null, 2));
  }, [errors, formData]);

  // Handle form submission
  const onSubmit = async (formData: StepTwoData) => {
    console.log('StepTwo onSubmit:', JSON.stringify(formData, null, 2));
    updateData(formData);
    return true;
  };
  ...
  React.useEffect(() => {
    ...
    triggerSubmit(async () => {
      ...
      console.log('StepTwo isValid:', isValid);
      if (!isValid) {
        // console.log('StepTwo Validation Errors:', JSON.stringify(errors, null, 2));
      }
      if (isValid) {
        const currentData = watch();
        await handleSubmit(onSubmit)({ target: { elements: [] } } as any);
        ...
        {fields.map((field, index) => (
          <div key={field.id} className="mb-8 border-b pb-4">
            ...
            {errors.educations?.[index]?.institution?.message && (
              <span className="error">{String(errors.educations[index].institution.message)}</span>
            )}
            <FormInput
            ...
            {errors.educations?.[index]?.degree?.message && (
              <span className="error">{String(errors.educations[index].degree.message)}</span>
            )}
            <FormInput
              id={`educations[${index}].field`}
              label="التخصص"
              placeholder="أدخل التخصص"
              {...register(`educations.${index}.field` as const, {
                onChange: () => trigger(`educations.${index}.field`),
              })}
            />
            {errors.educations?.[index]?.field?.message && (
              <span className="error">{String(errors.educations[index].field.message)}</span>
            )}
            <FormInput
            ...
            {errors.educations?.[index]?.startDate?.message && (
              <span className="error">{String(errors.educations[index].startDate.message)}</span>
            )}
            <FormInput
            ...
            {errors.educations?.[index]?.endDate?.message && (
              <span className="error">{String(errors.educations[index].endDate.message)}</span>
            )}
            <FormInput
