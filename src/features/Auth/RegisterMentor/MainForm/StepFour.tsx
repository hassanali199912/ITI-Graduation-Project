    ...
    <div className="step-four mx-4">
      ...
      <form className="my-16">
        <SelectInput
          ...
          multiple
          {...register('teachingAreaIds', { onChange: () => trigger('teachingAreaIds') })}
        />
        ...
        <SelectInput
          ...
          multiple
          {...register('ageGroupIds', { onChange: () => trigger('ageGroupIds') })}
        />
        {errors.ageGroupIds?.message && <span className="error">{String(errors.ageGroupIds.message)}</span>}
        <SelectInput
          id="communicationMethodIds"
          label="كيف تفضل التواصل مع المتعلمين؟"
          options={communicationMethods.map((c) => ({ id: c.id, name: c.name }))}
          multiple
          {...register('communicationMethodIds', { onChange: () => trigger('communicationMethodIds') })}
        />
        ...
        <SelectInput
          ...
          multiple
          {...register('teachingLanguageIds', { onChange: () => trigger('teachingLanguageIds') })}
        />
        ...
        {fields.map((field, index) => (
          <div key={field.id} className="mb-4">
            <FormInput
              ...
              {...register(`additionalInterests.${index}` as const, {
                onChange: () => trigger(`additionalInterests.${index}`),
              })}
            />
            {fields.length > 1 && (
              <button
